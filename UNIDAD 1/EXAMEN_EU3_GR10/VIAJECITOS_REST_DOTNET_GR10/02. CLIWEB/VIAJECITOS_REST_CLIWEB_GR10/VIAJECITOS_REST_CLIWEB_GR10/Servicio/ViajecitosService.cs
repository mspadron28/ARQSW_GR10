...
namespace VIAJECITOS_REST_CLIWEB_GR10.Servicio
{
    public class ViajecitosService
    {
        ...
        public ViajecitosService(IHttpClientFactory httpClientFactory)
        {
            ...
            Console.WriteLine($"[DEBUG] HttpClient inicializado con BaseUrl: {_baseUrl}");
        }
        ...
        public async Task<List<Vuelo>> BuscarVuelos(string ciudadOrigen, string ciudadDestino, DateTime fecha)
        {
            try
            {
                ...
                Console.WriteLine($"[DEBUG] Enviando GET a: {_baseUrl}{url}");
        ...
        public async Task<Cliente> RegistrarCliente(string nombre, string email, string documentoIdentidad)
        {
            try
            {
                ...
                Console.WriteLine($"[DEBUG] Enviando POST a: {_baseUrl}{url}");
        ...
        public async Task<Factura> CrearFactura(string numeroFactura, int idEmpleado, int idCliente, int idMetodoPago, decimal descuento, List<(int IdVuelo, int Cantidad)> detalles)
        {
            try
            {
                ...
                Console.WriteLine($"[DEBUG] Enviando POST a: {_baseUrl}{url}, Contenido: {json}");
            ...
            {
                Console.WriteLine($"[ERROR] HttpRequestException en CrearFactura: {ex.Message}, InnerException: {ex.InnerException?.Message}");
                throw new Exception($"Error en la solicitud HTTP: {ex.Message}", ex);
            }
            catch (JsonException ex)
            {
                Console.WriteLine($"[ERROR] JsonException en CrearFactura: {ex.Message}");
                throw new Exception($"Error al deserializar la respuesta: {ex.Message}", ex);
            }
            catch (Exception ex)
            {
